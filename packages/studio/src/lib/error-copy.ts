const KNOWN_RUNTIME_REPLACEMENTS: ReadonlyArray<{
  ...
  {
    pattern: /MINBOOK_LLM_API_KEY not set\. Run 'minbook config set-global' or add it to project \.env file\./g,
    replacement: "MINBOOK_LLM_API_KEY 未设置。请运行 `minbook config set-global`，或在项目 .env 文件中添加它。",
  },
];
