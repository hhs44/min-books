...
describe("secrets", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-secrets-"));
  });
  ...
  describe("loadSecrets", () => {
    it("returns empty when .minbook/secrets.json does not exist", async () => {
  ...
  describe("saveSecrets", () => {
    it("creates .minbook dir and writes secrets file", async () => {
      await saveSecrets(root, {
        services: { deepseek: { apiKey: "sk-deep" } },
      });
      const raw = await readFile(join(root, ".minbook", "secrets.json"), "utf-8");
