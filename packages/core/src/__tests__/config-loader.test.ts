...
const ENV_KEYS = [
  "MINBOOK_LLM_SERVICE",
  "MINBOOK_LLM_PROVIDER",
  "MINBOOK_LLM_BASE_URL",
  "MINBOOK_LLM_MODEL",
  "MINBOOK_LLM_API_KEY",
  "MINBOOK_LLM_TEMPERATURE",
  "MINBOOK_LLM_THINKING_BUDGET",
  "MINBOOK_LLM_API_FORMAT",
  "MINBOOK_LLM_STREAM",
  "MINBOOK_LLM_EXTRA_top_p",
  "MINBOOK_DEFAULT_LANGUAGE",
] as const;
...
describe("loadProjectConfig local provider auth", () => {
  ...
  it("allows missing API keys for localhost OpenAI-compatible endpoints", async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-config-loader-local-"));
    for (const key of ENV_KEYS) {
  ...
  it("still requires API keys for remote hosted endpoints", async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-config-loader-remote-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
    ...
    await writeFile(join(root, ".env"), "", "utf-8");
    await expect(loadProjectConfig(root)).rejects.toThrow(/MINBOOK_LLM_API_KEY not set/i);
  });

  it("loads service-based config using defaultModel and project secrets", async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-config-loader-services-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
  ...
  it("derives provider/baseUrl from the MiniMax preset single source of truth", async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-config-loader-minimax-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
  ...
  it("loads custom service config using custom secret key and entry baseUrl", async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-config-loader-custom-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
  ...
  it("keeps Studio config active when llm.configSource is studio", async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-config-loader-studio-source-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
    ...
    await writeFile(join(root, ".env"), [
      ...
      "MINBOOK_LLM_BASE_URL=https://api-vip.codex-for.me/v1",
      "MINBOOK_LLM_MODEL=gpt-5.4",
  ...
  it("does not mix stale top-level env-era model/baseUrl with selected Studio service", async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-config-loader-studio-stale-top-level-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
  ...
  it("falls back to env when Studio config is still the empty bootstrap state", async () => {
    root = await mkdtemp(join(tmpdir(), "minbook-config-loader-studio-bootstrap-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }

    process.env.MINBOOK_LLM_PROVIDER = "openai";
    process.env.MINBOOK_LLM_BASE_URL = "https://api-vip.codex-for.me/v1";
    process.env.MINBOOK_LLM_MODEL = "gpt-5.4";
    process.env.MINBOOK_LLM_API_KEY = "sk-env";
