...
const ZH_CN: TuiCopy = {
  ...
  composer: {
    placeholder: "告诉 MinBook 要写什么、修改什么，或解释什么…",
    emptyConversation: "先告诉 MinBook 你要做什么。",
  ...
  notes: {
    ...
    config: "当前 Ink 仪表盘里还不支持交互式 /config。请使用 minbook config set-global。",
...
const EN: TuiCopy = {
  ...
  composer: {
    placeholder: "Ask MinBook to write, revise, or explain…",
    emptyConversation: "Start by asking MinBook what to do.",
  ...
  notes: {
    ...
    config: "Interactive /config is not available inside the Ink dashboard yet. Use minbook config set-global.",
...
export function resolveTuiLocale(
  ...
  const requested = normalizeLocale(env.MINBOOK_TUI_LOCALE ?? env.MINBOOK_LOCALE);
  if (requested) {
    return requested;
  }
