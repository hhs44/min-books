...
export const updateCommand = new Command("update")
  .description("Update MinBook to the latest version")
  .action(async () => {
    try {
      ...
      const remoteVersion = execSync("npm view @hhs44/minbook version", {
        encoding: "utf-8",
      }).trim();
      ...
      execSync("npm install -g @hhs44/minbook@latest", { stdio: "inherit" });
      log(`Updated to ${remoteVersion}.`);
    } catch (e) {
      logError(`Update failed: ${e}`);
      log("You can also update manually: npm install -g @hhs44/minbook@latest");
      process.exit(1);
    }
  });
