...
export { extractResponsesImageBase64, resolveCoverApiKey } from "@hhs44/minbook-core";
...
shortCommand
  ...
  .option("--cover-api-key-env <name>", "Env var containing cover API key", "MINBOOK_COVER_API_KEY")
...
async function createShortRuntime(
  ...
  try {
    ...
    if (!String(e).includes("minbook.json not found")) throw e;
...
function buildEnvLLMConfig(options: {
  ...
  const baseUrl = options.llmBaseUrl ?? process.env.MINBOOK_LLM_BASE_URL;
  const model = options.model ?? process.env.MINBOOK_LLM_MODEL;
  if (!baseUrl) throw new Error("LLM base URL is required. Set MINBOOK_LLM_BASE_URL or pass --llm-base-url.");
  if (!model) throw new Error("LLM model is required. Set MINBOOK_LLM_MODEL or pass --model.");
  return {
    provider: "openai",
    service: process.env.MINBOOK_LLM_SERVICE ?? "custom",
    configSource: "env",
    baseUrl,
    apiKey: process.env.MINBOOK_LLM_API_KEY ?? "",
    model,
    temperature: parseEnvNumber(process.env.MINBOOK_LLM_TEMPERATURE, 0.1),
    thinkingBudget: parseEnvInteger(process.env.MINBOOK_LLM_THINKING_BUDGET, 0),
    apiFormat: process.env.MINBOOK_LLM_API_FORMAT === "responses" ? "responses" : "chat",
    stream: process.env.MINBOOK_LLM_STREAM === "false" ? false : true,
  };
}
