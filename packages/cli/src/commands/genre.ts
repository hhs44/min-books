...
import { listAvailableGenres, readGenreProfile, getBuiltinGenresDir } from "@hhs44/minbook-core";
  ...
  .action(async (id: string) => {
    try {
      ...
      try {
        ...
        logError(`Built-in genre "${id}" not found. Use 'minbook genre list' to see available genres.`);
        process.exit(1);
        return;
      }
