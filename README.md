...
AI Agent 自主写小说——写、审、改，全程接管。覆盖玄幻、仙侠、都市、科幻等多种风格，支持续写、番外、同人、仿写等创作形式。人工审核门控确保你始终掌控全局。已发布为 [OpenClaw](https://clawhub.ai/narcooo/minbook) skill。
...
**MinBook Studio 2.0 正式发布！** — 直接运行 `minbook` 启动本地 Web 工作台。书籍管理、章节审阅编辑、实时写作进度、市场雷达、数据分析、AI 检测、文风分析、题材管理、守护进程控制、真相文件编辑——CLI 能做的，Studio 全部可视化。

**MinBook TUI 正式发布！** — 运行 `minbook tui` 进入全屏交互仪表盘。对话式创作、自然语言操作书籍、slash 命令补全、主题动效——TUI、Studio、OpenClaw 共享同一套交互内核。

**MinBook Short** — Studio 对话和 CLI 现在可以直接产出独立短篇：完整正文、大纲记录、审稿记录、简介卖点、封面提示词，并在配置封面服务后生成封面图。
...
<p align="center">
  <img src="assets/minbook-short-demo-cover.png" width="320" alt="MinBook Short 短篇封面示例">
</p>
...
MinBook 已发布为 [OpenClaw](https://clawhub.ai/narcooo/minbook) Skill，可被任何兼容 Agent（Claude Code、OpenClaw 等）直接调用：

```bash
clawhub install minbook          # 从 ClawHub 安装 MinBook Skill
```

通过 npm 安装或克隆本项目时，`skills/SKILL.md` 已包含在内，🦞 可直接读取——无需额外从 ClawHub 安装。

安装后，Claw 应优先通过共享交互入口调用 MinBook：

```bash
minbook interact --json --message "继续当前书，但把节奏再收紧一点"
```
...
`plan chapter` / `compose chapter` / `draft` / `audit` / `revise` / `write next` 这些原子命令仍然保留，但更适合作为底层工具，而不是 OpenClaw 的首选入口。也可以在 [ClawHub](https://clawhub.ai) 搜索 `minbook` 在线查看。

### 配置

MinBook 2.0 将 LLM 配置分成两条清晰路径：**Studio 用可视化服务配置**，**CLI / daemon / 部署环境支持 env 覆盖**。两者不会互相污染。
...
```text
provider bank 默认值
→ minbook.json 里的 services / 当前 service / defaultModel
→ .minbook/secrets.json 里的 service API Key
```

即使检测到 `~/.minbook/.env` 或项目 `.env`，Studio 也只会展示提示，不会用 env 覆盖 service、model、baseUrl 或 API Key。API Key 存在项目内的 `.minbook/secrets.json`，不会写进 `minbook.json`。
...
也可以手动写 `~/.minbook/.env` 或项目 `.env`：

```bash
MINBOOK_LLM_PROVIDER=custom
MINBOOK_LLM_BASE_URL=https://api.moonshot.cn/v1
MINBOOK_LLM_API_KEY=sk-...
MINBOOK_LLM_MODEL=kimi-k2.5

# 可选
MINBOOK_LLM_SERVICE=moonshot                         # 推荐写；不写时会尽量从 baseUrl 自动识别
MINBOOK_LLM_TEMPERATURE=0.7
MINBOOK_LLM_THINKING_BUDGET=0
MINBOOK_DEFAULT_LANGUAGE=zh
...
→ .minbook/secrets.json service key
→ global ~/.minbook/.env
→ project .env
→ 当前进程环境变量
→ CLI 参数
...
也就是说，CLI 默认可以复用 Studio 配好的服务和密钥；如果 env 里声明了 `MINBOOK_LLM_SERVICE`、`MINBOOK_LLM_MODEL`、`MINBOOK_LLM_BASE_URL` 或 `MINBOOK_LLM_API_KEY`，则作为覆盖层生效。旧 env 只写 `baseUrl + model + apiKey` 也能继续用，MinBook 会尽量从 baseUrl 反推 service。

一次性指定服务或模型：

```bash
minbook write next --service google --model gemini-2.5-flash
minbook write next --service moonshot --model kimi-k2.5 --no-stream
minbook agent "继续写下一章" --api-key-env MOONSHOT_API_KEY
minbook doctor --service minimaxCodingPlan --model MiniMax-M2.7
```
...
minbook config show-models        # 查看当前路由
```
...
如果服务测试失败，优先检查服务商、模型和协议是否匹配。Google Gemini 的 AI Studio API Key 可用于 Gemini OpenAI-compatible endpoint；MinBook 会自动禁用 Google 不支持的 OpenAI `store` 参数。MiniMax / MiniMax CodingPlan 默认走官方 OpenAI-compatible `/v1/chat/completions`，并优先使用可工作的非流式 transport，避免流式返回 usage 但无正文的问题。

### v2.0 LLM 配置更新

- **Studio / CLI 配置隔离**：Studio 固定使用服务页配置和 `.minbook/secrets.json`；CLI、daemon、部署环境支持 env 覆盖和一次性命令参数。
...
- **Google Gemini 兼容修复**：AI Studio API Key 可直接用于 Gemini OpenAI-compatible endpoint，MinBook 会自动禁用 Google 不支持的 OpenAI `store` 参数。
...
- **旧 env 兼容**：老的 `MINBOOK_LLM_BASE_URL + MINBOOK_LLM_MODEL + MINBOOK_LLM_API_KEY` 仍可用于 CLI；没有 `MINBOOK_LLM_SERVICE` 时会尝试从 baseUrl 反推服务商。

### v1.2 更新

**统一交互内核 + TUI 仪表盘 + Studio 助手**

- **共享交互运行时**：TUI、Studio、`minbook interact`、OpenClaw Skill 共用同一套自然语言理解 + 执行内核，支持 15+ 种意图（写作、修订、重写、改名、导出、切换书籍等）
- **Ink TUI 仪表盘**：`minbook` 直接进入全屏交互式仪表盘（Ink + React），对话式创作体验，slash 命令自动补全，主题动效，i18n 双语
...
- **`minbook interact`**：共享交互 JSON 入口，OpenClaw / 外部 Agent 可直接调用
...
```bash
minbook book create --title "吞天魔帝" --genre xuanhuan  # 创建新书
minbook write next 吞天魔帝      # 写下一章（草稿 → 审计 → 按配置修订）
minbook status                   # 查看状态
minbook review list 吞天魔帝     # 审阅草稿
minbook review approve-all 吞天魔帝  # 批量通过
minbook export 吞天魔帝          # 导出全书
minbook export 吞天魔帝 --format epub  # 导出 EPUB（手机/Kindle 阅读）
```
...
`minbook style analyze` 分析参考文本，提取统计指纹（句长分布、词频特征、节奏模式）和 LLM 风格指南。`minbook style import` 将指纹注入指定书籍，后续所有章节自动采用该风格，修订者也会用风格标准做审计。

### 创作简报

`minbook book create --brief my-ideas.md` 传入你的脑洞、世界观设定、人设文档。建筑师 agent 会基于简报生成故事设定（`story_bible.md`）和创作规则（`book_rules.md`），而非凭空创作；同时把简报落盘到 `story/author_intent.md`，让这本书的长期创作意图不会只在建书时生效一次。
...
```bash
minbook plan chapter 吞天魔帝 --context "本章先把注意力拉回师徒矛盾"
minbook compose chapter 吞天魔帝
```
...
- 如果正文超出允许区间，MinBook 最多只会追加 1 次纠偏归一化（压缩或补足），不会直接硬截断正文
- 如果 1 次纠偏后仍然超出 hard range，章节照常保存，但会在结果和 chapter index 里留下长度 warning / telemetry

### 续写已有作品

`minbook import chapters` 从已有小说文本导入章节，自动逆向工程 7 个真相文件（世界状态、角色矩阵、资源账本、伏笔钩子等），支持 `第X章` 和自定义分割模式、断点续导。导入后 `minbook write next` 无缝接续创作。

### 同人创作

`minbook fanfic init --from source.txt --mode canon` 从原作素材创建同人书。支持四种模式：canon（正典延续）、au（架空世界）、ooc（性格重塑）、cp（CP 向）。内置正典导入器、同人专属审计维度和信息边界管控——确保设定不矛盾。

### 多模型路由

不同 Agent 可以走不同模型和 Provider。写手用 Claude（创意强），审计用 GPT-4o（便宜快速），雷达用本地模型（零成本）。`minbook config set-model` 按 agent 粒度配置，未配置的自动回退全局模型。

### 守护进程 + 通知推送

`minbook up` 启动后台循环自动写章。管线对非关键问题全自动运行，关键问题暂停等人工审核。通知推送支持 Telegram、飞书、企业微信、Webhook（HMAC-SHA256 签名 + 事件过滤）。日志写入 `minbook.log`（JSON Lines），`-q` 静默模式。

### 本地模型兼容

支持任何 OpenAI 兼容接口（Studio 里新增自定义服务，或 CLI 使用 `--provider custom` / `MINBOOK_LLM_PROVIDER=custom`）。服务测试会尝试不同协议和流式开关组合，并保存或提示可用 transport。Fallback 解析器处理小模型不规范输出，流中断时自动恢复部分内容。

### 可靠性保障

每章自动创建状态快照，`minbook write rewrite` 可回滚任意章节。写手动笔前输出自检表（上下文、资源、伏笔、风险），写完输出结算表，审计员交叉验证。文件锁防止并发写入。写后验证器含跨章重复检测和 11 条硬规则自动 spot-fix。
...
模型输出上限由 provider bank 的模型卡管理；`llm.extra` / `MINBOOK_LLM_EXTRA_*` 中的保留键（max_tokens、temperature、model、messages、stream 等）会被自动过滤，防止意外覆盖核心请求参数。
...
如果审计不通过，默认管线只做一次"修订 → 再审计"；仍未解决的问题会保留在结果和状态里，交给人工或后续命令继续处理。需要更强自动闭环时，可以运行 `minbook config set writing.reviewRetries 3` 把修订轮数调高。
...
除了 7 个真相文件，MinBook 还把“护栏”和“自定义”拆成可审阅的控制层：
...
MinBook 提供三种交互方式，底层共享同一组原子操作：

### 1. 完整管线（一键式）

```bash
minbook write next 吞天魔帝          # 写草稿 → 审计 → 按配置自动修订
minbook write next 吞天魔帝 --count 5 # 连续写 5 章
```

`write next` 现在默认走 `plan -> compose -> write` 的输入治理链路，审计后的自动修订轮数默认是 1。若你需要回退到旧的 prompt 拼装路径，可在 `minbook.json` 中显式设置：
...
```bash
minbook plan chapter 吞天魔帝 --context "本章重点写师徒矛盾" --json
minbook compose chapter 吞天魔帝 --json
minbook draft 吞天魔帝 --context "本章重点写师徒矛盾" --json
minbook audit 吞天魔帝 31 --json
minbook revise 吞天魔帝 31 --json
```
...
```bash
minbook agent "帮我写一本都市修仙，主角是个程序员"
minbook agent "写下一章，重点写师徒矛盾"
minbook agent "先扫描市场趋势，然后根据结果创建一本新书"
```
...
| 命令 | 说明 |
|------|------|
| `minbook init [name]` | 初始化项目（省略 name 在当前目录初始化） |
| `minbook book create` | 创建新书（`--genre`、`--platform`、`--chapter-words`、`--target-chapters`、`--brief <file>` 传入创作简报） |
| `minbook book update [id]` | 修改书设置（`--chapter-words`、`--target-chapters`、`--status`） |
| `minbook book list` | 列出所有书籍 |
| `minbook book delete <id>` | 删除书籍及全部数据（`--force` 跳过确认） |
| `minbook genre list/show/copy/create` | 查看、复制、创建题材 |
| `minbook plan chapter [id]` | 生成下一章的 `intent.md`（`--context` / `--context-file` 传入当前指令） |
| `minbook compose chapter [id]` | 生成下一章的 `context.json`、`rule-stack.yaml`、`trace.json` |
| `minbook write next [id]` | 完整管线写下一章（`--words` 覆盖字数，`--count` 连写，`-q` 静默模式） |
| `minbook write rewrite [id] <n>` | 重写第 N 章（恢复状态快照，`--force` 跳过确认，`--words` 覆盖字数） |
| `minbook draft [id]` | 只写草稿（`--words` 覆盖字数，`-q` 静默模式） |
| `minbook audit [id] [n]` | 审计指定章节 |
| `minbook revise [id] [n]` | 修订指定章节 |
| `minbook agent <instruction>` | 自然语言 Agent 模式 |
| `minbook review list [id]` | 审阅草稿 |
| `minbook review approve-all [id]` | 批量通过 |
| `minbook status [id]` | 项目状态 |
| `minbook export [id]` | 导出书籍（`--format txt/md/epub`、`--output <path>`、`--approved-only`） |
| `minbook radar scan` | 扫描平台趋势 |
| `minbook fanfic init` | 从原作素材创建同人书（`--from`、`--mode canon/au/ooc/cp`） |
| `minbook config set-global` | 设置 CLI / daemon / 部署环境的全局 LLM env（`~/.minbook/.env`） |
| `minbook config show-global` | 查看全局配置 |
| `minbook config set/show` | 查看/更新项目配置 |
| `minbook config set-model <agent> <model>` | 为指定 agent 设置模型覆盖（`--base-url`、`--provider`、`--api-key-env` 支持多 Provider 路由） |
| `minbook config remove-model <agent>` | 移除 agent 模型覆盖（回退到默认） |
| `minbook config show-models` | 查看当前模型路由 |
| `minbook doctor` | 诊断配置问题（显示 effective config mode、来源、API 连通性和提供商兼容性提示） |
| `minbook detect [id] [n]` | AIGC 检测（`--all` 全部章节，`--stats` 统计） |
| `minbook style analyze <file>` | 分析参考文本提取文风指纹 |
| `minbook style import <file> [id]` | 导入文风指纹到指定书 |
| `minbook import canon [id] --from <parent>` | 导入正传正典到番外书 |
| `minbook import chapters [id] --from <path>` | 导入已有章节续写（`--split`、`--resume-from`） |
| `minbook analytics [id]` / `minbook stats [id]` | 书籍数据分析（审计通过率、高频问题、章节排名、token 用量） |
| `minbook update` | 更新到最新版本 |
| `minbook studio` / `minbook` | 启动 Web 工作台（`-p` 指定端口，默认 4567；Studio 使用服务页配置，不使用 env 覆盖） |
| `minbook up / down` | 启动/停止守护进程（`-q` 静默模式，自动写入 `minbook.log`） |
...
```bash
minbook write next --service google --model gemini-2.5-flash
minbook up --service moonshot --model kimi-k2.5 --api-key-env MOONSHOT_API_KEY
```

## 路线图

- [x] ~~`packages/studio` Web UI 工作台（Vite + React + Hono）~~ — 已发布，`minbook studio` 启动
- [ ] 互动小说（分支叙事 + 读者选择）
