...
- 长篇章节写作的自动审稿修稿轮数新增 `writing.reviewRetries` 项目配置，默认仍为 1；需要更强修稿时可执行 `minbook config set writing.reviewRetries 3`
...
- Chat 可直接编辑项目内生成文本产物，适合调整章节、封面提示词、简介等文件后再继续使用 MinBook 写作链路
...
- **Windows Studio 启动修复**：`minbook studio` 在 Windows 下不再因绝对路径 loader 被当成非法 ESM URL 而崩溃
- **Bootstrap 项目回退到 env 配置**：空目录 auto-init 后的 Studio 项目，在未配置服务时会回退到全局 `.minbook/.env`，`book create` 不再先死在缺 key
...
- **空目录直接启动**：`minbook` / `minbook studio` 现在会自动初始化最小项目骨架并启动 Studio，不再要求显式先跑 `init`
...
Studio 2.0 正式发布。`minbook` 现在默认直接启动 Studio，本地 Web 工作台成为主入口；TUI 保留为 `minbook tui`。

### 新功能

- **Studio 2.0 默认入口**：`minbook` 直接启动 Studio，首页、服务商管理、写作工作台统一为新的主交互入口
- **自定义 OpenAI-compatible 服务**：Studio 现支持自定义 `baseUrl`、协议类型（`chat` / `responses`）与流式开关，兼容更多中转站和聚合网关
- **配置来源切换**：Studio 新增 `.env` 与 Studio 配置的显式切换，不再只能被目录里的 `MINBOOK_LLM_*` 被动覆盖
...
- 修复 `minbook` 启动 Studio 时因未设置默认模型而直接抛出 `llm.model` 校验错误
...
统一交互内核——TUI、Studio、`minbook interact`、OpenClaw Skill 共享同一套自然语言理解和执行运行时。

### 新功能

- **共享交互运行时**（`packages/core/src/interaction/`）：自然语言路由器（15+ intent）、会话管理、编辑事务控制器、事件追踪、阶段遥测
- **Ink TUI 仪表盘**：`minbook` 直接进入全屏 Ink + React 仪表盘，对话式创作，slash 命令 Tab 补全，主题动效（writing/auditing/revising/planning 各有独立动画），i18n 中英双语
...
- **`minbook interact --json`**：共享交互 JSON 入口，返回 request / response / session / events，供 OpenClaw 和外部 Agent 直接调用
...
- **Review Reject 回滚**：`minbook review reject` 回滚 state 到被拒章节之前的快照，丢弃下游章节和记忆索引
- **State Validation Recovery**：state 校验失败自动重试 settler，仍失败则降级保存，支持 `minbook write repair-state` 手动修复
...
- **自定义 HTTP Headers**：`MINBOOK_LLM_HEADERS` 环境变量注入自定义 HTTP 头
...
MinBook Studio + 稳定性加固。从 CLI 工具升级为 CLI + Web 工作台。

### MinBook Studio

- `minbook studio` 启动本地 Web 工作台（Vite + React + Hono，默认端口 4567）
...
- 用户 `MINBOOK_LLM_MAX_TOKENS` 作为全局上限生效（#87）
...
- `minbook status --chapters`：显示每章状态和 failed 章节的 critical issues
- 审计 JSON 解析容错（#51）
...
- `minbook up` 自动写入 `minbook.log`，守护进程重启后可追溯
...
- `minbook doctor` 失败时给出针对性 hints（检查 baseUrl、试 stream:false、检查 API Key）
...
把已有的小说（单文件或章节目录）导入 MinBook，系统自动拆章、逆向工程生成全套真相文件（世界状态、伏笔、角色矩阵等），之后直接 `write next` 续写。

```bash
minbook import chapters 我的小说 --from 已有章节/        # 从目录导入
minbook import chapters 我的小说 --from 全书.txt          # 从单文件导入（自动按"第X章"拆分）
minbook import chapters 我的小说 --from 全书.txt --split "Chapter\\s+\\d+"  # 自定义分章正则
minbook write next 我的小说                               # 无缝续写
```
...
```bash
minbook import canon 烈焰前传 --from 吞天魔帝   # 导入正传正典到番外
minbook write next 烈焰前传                     # 写手自动读取正典约束
```
...
```bash
minbook style analyze 参考小说.txt                     # 分析：句长、TTR、修辞特征
minbook style import 参考小说.txt 吞天魔帝 --name 某作者  # 导入文风到书
```
...
```bash
minbook config set-model writer gpt-4o-mini                                    # 简单模型覆盖
minbook config set-model auditor gemini-2.5-flash \
  --base-url https://generativelanguage.googleapis.com/v1beta/openai \
  --provider openai \
  --api-key-env GEMINI_API_KEY                                                # 走 Gemini API
minbook config set-model reviser claude-sonnet-4-20250514 \
  --base-url https://api.anthropic.com \
  --provider anthropic \
  --api-key-env ANTHROPIC_API_KEY                                             # 走 Anthropic API
minbook config show-models                                                      # 查看路由全景
```
...
```bash
minbook analytics 吞天魔帝          # 审计通过率、高频问题类别、问题最多的章节
minbook analytics 吞天魔帝 --json   # 结构化输出
```
...
- `minbook export` 支持 `--format md`、`--output <path>`、`--approved-only`
...
| AIGC 检测 API | 外部 API 集成（GPTZero / Originality / 自定义端点），`minbook detect` 命令 |
| 文风指纹学习 | 从参考文本提取 StyleProfile（句长、TTR、修辞特征），注入 Writer prompt |
| 反检测改写 | ReviserAgent `anti-detect` 模式，检测→改写→重检测循环 |
| 检测反馈闭环 | `detection_history.json` 记录每次检测/改写结果，`minbook detect --stats` 查看统计 |

```bash
minbook style analyze reference.txt         # 分析参考文本文风
minbook style import reference.txt 吞天魔帝  # 导入文风到书
minbook detect 吞天魔帝 --all               # 全书 AIGC 检测
minbook detect --stats                      # 检测统计
```
...
```bash
minbook book create --title "吞天魔帝" --genre xuanhuan
```

题材规则可以查看、复制到项目中修改、或从零创建：

```bash
minbook genre list                      # 查看所有题材
minbook genre show xuanhuan             # 查看玄幻的完整规则
minbook genre copy xuanhuan             # 复制到项目中，随意改
minbook genre create wuxia --name 武侠   # 从零创建新题材
```
...
- `minbook update` 一键更新、`minbook init` 支持当前目录初始化
- API 错误附带中文诊断提示，`minbook doctor` 含 API 连通性测试
